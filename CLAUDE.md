...
- `VerificationReport` - Analyzes project for release readiness: external SNAPSHOT dependencies in all four dependency sections of the root and every workspace package (`workspace:` specifiers ignored, `catalog:` specifiers resolved via `Workspace.resolveCatalogSpecifier()`). Failing checks feed `hasFailures()`; each check lives in its own section of the class
