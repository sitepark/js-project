...
import { parsePackageManagerArg } from "./packageManager.js";
...
yargs(hideBin(process.argv))
  ...
  .command(
    ...
    (argv) => {
      try {
        verifyReleaseCommand(parsePackageManagerArg(argv));
      } catch (error) {
        handleError(error, argv.verbose);
      }
    },
  ...
  .command(
    ...
    (argv) => {
      try {
        startHotfixCommand(parsePackageManagerArg(argv), argv.tag);
      } catch (error) {
        handleError(error, argv.verbose);
      }
    },
  ...
  .command(
    ...
    async (argv) => {
      try {
        await publishCommand(parsePackageManagerArg(argv));
      } catch (error) {
        handleError(error, argv.verbose);
      }
    },
